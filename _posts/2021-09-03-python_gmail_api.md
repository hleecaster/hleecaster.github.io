...
> 2026년 7월 기준 글에 적힌 화면 안내는 지금과 다르다. Google Cloud Console의 메뉴 구성과 OAuth 동의 화면 설정 절차가 그 사이 여러 번 개편됐다. 파이썬 코드의 큰 흐름(인증 → 메시지 생성 → 발송)은 그대로지만, 콘솔에서 인증 정보를 만드는 부분은 공식 문서를 다시 봐야 한다.
{: .prompt-warning }
