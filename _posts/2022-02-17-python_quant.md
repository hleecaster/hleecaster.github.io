...
> 2026년 7월 기준 이 예제는 그대로 돌아가지 않는다. `pandas_datareader`의 야후 파이낸스 연동이 폐기되어 `web.DataReader(ticker, 'yahoo', ...)` 부분에서 막힌다. 또 `df.fillna(method='ffill')`의 `method` 인자는 pandas 2.x에서 제거됐다. 지표를 계산하는 로직 자체는 유효하니, 데이터를 받아오는 부분만 `yfinance` 같은 걸로 바꿔야 한다.
{: .prompt-warning }
