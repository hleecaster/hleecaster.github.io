...
이전에 [선형회귀에 대한 개념](/posts/ml_linear_regression_concepts/)을 소개한 바 있다. X의 값에 따라 Y값이 어떻게 달라질지 예측하는, 기울기와 절편만 있는 단순한 1차 방정식 `y = m*X + b`로 설명했는데, 이건 그냥 단순선형회귀라고 부른다.
...
4. **다중선형회귀(Multiple Linear Regression) 파이썬 코드 예시** &nbsp;← 현재 글
