...
> 2026년 7월 기준 이 글의 명령어는 맞지 않는다. 본문은 ngrok 2.x를 기준으로 쓰였는데, 3.x에서 인증 토큰 등록이 `ngrok authtoken <토큰>`에서 `ngrok config add-authtoken <토큰>`으로 바뀌었고 `-subdomain=` 옵션도 없어졌다. 설치 방법과 가입 화면도 지금과 다르다. 로컬 서버를 외부에 노출한다는 개념은 그대로 유효하다.
{: .prompt-warning }
