...
흔히 파이썬 웹 프레임워크로 [Django](https://www.djangoproject.com/)(장고) 아니면 [Flask](https://flask.palletsprojects.com/en/stable/)(플라스크)를 많이 사용하는 것 같다. Django는 이런저런 백엔드에서 필요한 다양한 기능을 빵빵하게 지원하고 풀 스택 웹 프레임워크인 반면, Flask는 정말 최소한의 수준으로만 구현이 되어있는 마이크로 프레임워크라고 한다. 나 같은 초보 입장에서는 일단 최대한 가볍고 쉬운 Flask를 택할 수밖에 없었다.
